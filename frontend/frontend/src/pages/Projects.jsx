...
export default function Projects() {
  ...
  const { data: projects = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map(p => ({
        ...p,
        membersCount: p.project_members?.length || 0,
        taskStats: {
  ...
  const save = useMutation({
    mutationFn: async (d) => {
      ...
      delete payload.membersCount; delete payload.taskStats;
  ...
  const del = useMutation({
    ...
    onError: (e) => alert(e.message || 'Error al eliminar el proyecto')
  });
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {[['', 'Todos', counts.all], ['Planning','Planificación',counts.Planning],
          ...
          <button key={val} onClick={() => setFilter(val)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${filter === val ? 'bg-brand-500 text-white border-brand-600' : 'bg-surface-700 text-slate-400 border-surface-600 hover:text-slate-200'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <div key={p.id} className="card p-5 cursor-pointer hover:border-surface-400 transition-all group" onClick={() => navigate(`/projects/${p.id}`)}>
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0 pr-2">
                <h3 className="font-display text-lg font-bold text-white uppercase tracking-wide truncate group-hover:text-brand-400 transition-colors">
                  {p.name}
                </h3>
                <p className="text-slate-500 text-xs truncate">{p.client || 'Sin cliente'}</p>
              </div>
              <div className="flex gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
                <button className="btn-icon" onClick={() => { setForm({ ...p, budget: p.budget?.toString() || '' }); setModal(true); }}>
                  <Pencil size={13}/>
                </button>
        ...
        {shown.length === 0 && (
          <div className="col-span-3">
            <Empty icon={FolderKanban} title="Sin Proyectos" message="No hay proyectos que coincidan con el filtro seleccionado."
              action={<button className="btn-primary" onClick={() => { setForm(BLANK); setModal(true); }}><Plus size={14}/> Nuevo Proyecto</button>}/>
          </div>
        )}
      </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={form.id ? 'Editar Proyecto' : 'Nuevo Proyecto'} size="lg">
        <form onSubmit={e => { e.preventDefault(); save.mutate({ ...form, duration_weeks: parseInt(form.duration_weeks)||12, budget: parseFloat(form.budget)||0 }); }} className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <Field label="Nombre del Proyecto" required><input className="input" value={form.name || ''} onChange={e => setForm({...form, name: e.target.value})} required placeholder="Residencial Las Palmas…"/></Field>
          </div>
          <Field label="Cliente"><input className="input" value={form.client||''} onChange={e => setForm({...form, client: e.target.value})} placeholder="Nombre del cliente"/></Field>
          <Field label="Ubicación"><input className="input" value={form.location||''} onChange={e => setForm({...form, location: e.target.value})} placeholder="San José, Costa Rica"/></Field>
          ...
          <Field label="Presupuesto ($)"><input type="number" className="input" value={form.budget||''} min={0} step="0.01" onChange={e => setForm({...form, budget: e.target.value})} placeholder="0.00"/></Field>
          <Field label="Estado">
          ...
          <div className="col-span-2">
            <Field label="Descripción"><textarea className="input" rows={3} value={form.description||''} onChange={e => setForm({...form, description: e.target.value})} placeholder="Descripción del proyecto…"/></Field>
          </div>
          <div className="col-span-2 flex justify-end gap-2 pt-2">
            <button type="button" className="btn-ghost" onClick={() => setModal(false)}>Cancelar</button>
            <button type="submit" className="btn-primary" disabled={save.isPending}>{save.isPending ? 'Guardando…' : form.id ? 'Actualizar' : 'Crear Proyecto'}</button>
          </div>
        </form>
      </Modal>

      <Confirm open={!!delTgt} onClose={() => setDelTgt(null)} onConfirm={() => del.mutate(delTgt.id)} title="Eliminar Proyecto" message={`¿Eliminar "${delTgt?.name}"? Se eliminarán todas sus tareas y materiales asociados.`}/>
    </div>
  );
}
