  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-5">
        <div className={`card p-3 cursor-pointer transition-all text-center ${!fRole ? 'border-brand-500/50' : ''}`} onClick={() => setFRole('')}>
        ...
        {ROLES.map(r => (
          <div key={r} className={`card p-3 cursor-pointer transition-all text-center ${fRole===r ? 'border-brand-500/50' : ''}`} onClick={() => setFRole(fRole === r ? '' : r)}>
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className={`text-[10px] font-bold border px-1.5 py-0.5 rounded-full ${ROLE_BADGE[u.role]||ROLE_BADGE.Worker}`}>{u.role}</span>
                </div>
              </div>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button className="btn-icon" onClick={() => { setForm({...u, password:''}); setErr(''); setModal(true); }}><Pencil size={13}/></button>
                <button className="btn-icon hover:text-red-400" onClick={() => setDelTgt(u)}><Trash2 size={13}/></button>
              </div>
            </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={form.id ? 'Editar Empleado' : 'Nuevo Empleado'}>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          ...
          <Field label="Correo" required><input type="email" className="input" value={form.email || ''} onChange={e => setForm({...form,email:e.target.value})} required/></Field>
          <Field label={form.id ? 'Nueva Contraseña (opcional)' : 'Contraseña'} required={!form.id}>
            <input type="password" className="input" value={form.password || ''} onChange={e => setForm({...form,password:e.target.value})} required={!form.id} minLength={6} placeholder={form.id ? 'Dejar en blanco para no cambiar' : ''}/>
          </Field>
          <Field label="Rol">
          ...
          <div className="col-span-2 flex justify-end gap-2 pt-1">
            ...
            <button type="submit" className="btn-primary" disabled={save.isPending}>{save.isPending ? 'Guardando…' : form.id ? 'Actualizar' : 'Crear Empleado'}</button>
          </div>
        </form>
      </Modal>

      <Confirm open={!!delTgt} onClose={() => setDelTgt(null)} onConfirm={() => del.mutate(delTgt.id)} title="Eliminar Empleado" message={`¿Eliminar a "${delTgt?.name}"? Esta acción no se puede deshacer.`}/>
    </div>
  );
}
