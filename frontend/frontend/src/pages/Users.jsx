  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 gap-3 mb-5">
        <div className={`card p-3 cursor-pointer transition-all text-center ${!fRole?'border-brand-500/40':''}`} onClick={() => setFRole('')}>
      ...
      <div className="table-wrap">
        <table className="w-full">
          <thead>
            <tr>
              <th className="th">Usuario</th><th className="th">Correo</th><th className="th">Rol</th>
              <th className="th">Cargo</th><th className="th">Especialidad</th><th className="th">Teléfono</th>
              <th className="th">Creado</th><th className="th w-20"/>
            </tr>
          </thead>
          <tbody>
            {shown.map(u => (
              <tr key={u.id} className="tr-hover">
                ...
                <td className="td text-slate-500 text-xs font-mono">{u.created_at ? format(new Date(u.created_at),'dd/MM/yyyy') : '—'}</td>
                <td className="td">
                  <div className="flex gap-1">
                    <button className="btn-icon" onClick={() => { setForm({...u, password:''}); setErr(''); setModal(true); }}><Pencil size={12}/></button>
                    <button className="btn-icon hover:text-red-400" disabled={u.id === me?.id} title={u.id === me?.id ? 'No puedes eliminarte a ti mismo' : ''} onClick={() => u.id !== me?.id && setDelTgt(u)}><Trash2 size={12}/></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={form.id ? 'Editar Usuario' : 'Nuevo Usuario'}>
        <form onSubmit={e => { e.preventDefault(); save.mutate(form); }} className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <Field label="Nombre Completo" required><input className="input" value={form.name || ''} onChange={e => setForm({...form,name:e.target.value})} required/></Field>
          </div>
          <Field label="Correo Electrónico" required><input type="email" className="input" value={form.email || ''} onChange={e => setForm({...form,email:e.target.value})} required/></Field>
          <Field label={form.id ? 'Nueva Contraseña (opcional)' : 'Contraseña'} required={!form.id}>
            <input type="password" className="input" value={form.password || ''} onChange={e => setForm({...form,password:e.target.value})} required={!form.id} minLength={6} placeholder={form.id ? 'Dejar vacío para no cambiar' : 'Mínimo 6 caracteres'}/>
          </Field>
          <Field label="Rol">
          ...
          <Field label="Especialidad"><input className="input" value={form.specialty||''} onChange={e => setForm({...form,specialty:e.target.value})} placeholder="Estructuras, Electricidad…"/></Field>
          <Field label="Teléfono"><input className="input" value={form.phone||''} onChange={e => setForm({...form,phone:e.target.value})} placeholder="+506 8888-0000"/></Field>
          <div className="col-span-2 flex justify-end gap-2 pt-1">
            <button type="button" className="btn-ghost" onClick={() => setModal(false)}>Cancelar</button>
            <button type="submit" className="btn-primary" disabled={save.isPending}>{save.isPending ? 'Guardando…' : form.id ? 'Actualizar Usuario' : 'Crear Usuario'}</button>
          </div>
        </form>
      </Modal>

      <Confirm open={!!delTgt} onClose={() => setDelTgt(null)} onConfirm={() => del.mutate(delTgt.id)} title="Eliminar Usuario" message={`¿Eliminar al usuario "${delTgt?.name}"? Esta acción no se puede deshacer.`}/>
    </div>
  );
}
