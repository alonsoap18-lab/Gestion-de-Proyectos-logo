...
import { Plus, Trash2, Search, ShoppingCart, Calendar, MapPin, Download, CheckSquare, ArrowRight, ArrowLeft, PackageCheck } from 'lucide-react';
...
// Nuevos estados del ciclo de compra
const STATUSES = ['Pendiente', 'Aprobado', 'Enviado a Proveedor', 'Facturado Proveedor', 'Enviado', 'Recibido Sitio'];
...
export default function MaterialOrders() {
  ...
  const save = useMutation({
    mutationFn: async (d) => {
      ...
      // a) Procesar líneas manuales para meterlas al catálogo automáticamente
      const processedItems = await Promise.all(d.items.map(async (item) => {
        if (!item.catalog_id && item.manual_name?.trim()) {
          ...
          const { data: newCat, error: catErr } = await supabase.from('materials_catalog')
            ...
            .select().single();
          if (!catErr && newCat) {
            return { ...item, catalog_id: newCat.id, manual_name: null }; // Lo convertimos en item de catálogo
          }
        }
        return item;
      }));

      // b) Guardar Encabezado
    ...
    onSuccess: () => { 
      ...
      qc.invalidateQueries(['materials_catalog']); // Refresca por si hubo nuevos
      setModal(false); 
      setWizardStep(1);
    },
  ...
  const receiveOrder = useMutation({
    mutationFn: async (order) => {
      ...
      // 2. Preparar los datos para la tabla de inventario (materials)
      const inventoryItems = order.purchase_order_items.map(item => ({
        ...
        cost_per_unit: 0, // Inicia en 0, gerencia le pondrá precio después
      }));

      // 3. Insertar al inventario del proyecto
    ...
    onSuccess: () => { 
      qc.invalidateQueries(['purchase_orders']); 
      qc.invalidateQueries(['materials']); // Refresca el inventario global
      setReceiveTgt(null); 
    }
  });
  ...
  // Logica de Carrito (Wizard Step 2)
    ...
    return (
      <div className="flex items-center w-full mt-4 bg-surface-900/50 p-3 rounded-lg border border-surface-600/50">
        {STATUSES.map((status, index) => {
          ...
          // Nombres cortos para no saturar la tarjeta
    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        {filteredOrders.map(o => (
          <div key={o.id} className="card p-5 border border-surface-600 transition-all hover:border-surface-500 hover:shadow-xl relative overflow-hidden">
            {/* Decal de estado completado */}
            ...
            <div className="flex justify-between items-start mb-2">
              <div>
                <div className="text-brand-400 font-mono font-bold text-xl">{o.order_number}</div>
              ...
              <div className="flex gap-1 bg-surface-800 p-1 rounded-lg border border-surface-600">
                {/* Botón Mágico de Inventario */}
                {o.status !== 'Recibido Sitio' && (
        ...
        {filteredOrders.length === 0 && <div className="col-span-full py-12 text-center text-slate-500 bg-surface-800 rounded-xl border border-surface-600 border-dashed">No hay pedidos que coincidan con los filtros.</div>}
      </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={form.id ? `Editando ${form.order_number}` : 'Nuevo Pedido'} size="2xl">
        ...
        {wizardStep === 2 && (
          <div className="flex flex-col md:flex-row gap-5 h-[500px] animate-in fade-in slide-in-from-right-4">
            ...
            {/* PANEL IZQUIERDO: CATÁLOGO (Buscador) */}
      ...
      <Confirm open={!!receiveTgt} onClose={() => setReceiveTgt(null)} onConfirm={() => receiveOrder.mutate(receiveTgt)} 
        title="Recibir en Sitio" message={`¿Confirmas que el pedido ${receiveTgt?.order_number} llegó a obra? Esto registrará automáticamente todos los materiales en el inventario del proyecto.`}/>
